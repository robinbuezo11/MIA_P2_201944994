...
# def mkfs(id, fs):
def mkfs(id):
    result = 'Ejecutando el comando MKFS\n'
    result += '***** Buscando Particion *****\n'
    ...
    result += '***** Formateando Particion *****\n'
    # if fs == '2fs':
    res, msg = format_2fs(n, partition, path, superblock, date)
    if not res:
        result += f'No se pudo formatear la particion {partition.part_name}\n'
        result += msg
        return result
    # else:
        # if not format_3fs(n, partition, path, superblock, date):
            ...
            # return False
    result += f'Se formateo la particion {partition.part_name} con 2fs\n'
    result += '***** Finalizando MKFS *****\n'
    return result
...
def format_2fs(n, partition, path, superblock, date):
    ...
    try:
        ...
        msg = f'No se pudo abrir el disco {path}\n'
        return False, msg
    
    if not Fwrite_displacement(file, partition.part_start, superblock):
        msg = f'No se pudo escribir el SuperBloque en el disco {path}\n'
        file.close()
        return False, msg
    result = f'Se escribió el Super Bloque en el disco {path}\n'

    # Write Bitmap Inode
    bitmap = b'1' * 2 + b'0' * (n - 2)
    if not Fwrite_displacement_data(file, superblock.s_bm_inode_start, bitmap):
        result += f'No se pudo escribir el Bitmap de Inodos en el disco {path}\n'
        file.close()
        return False, result
    result += f'Se escribió el Bitmap de Inodos en el disco {path}\n'
    
    # Write Bitmap Block
    bitmap = b'1' * 2 + b'0' * (3 * n - 2)
    if not Fwrite_displacement_data(file, superblock.s_bm_block_start, bitmap):
        result += f'No se pudo escribir el Bitmap de Bloques en el disco {path}\n'
        file.close()
        return False, result
    result += f'Se escribió el Bitmap de Bloques en el disco {path}\n'
    
    # Write Inode Table
    inode = Inode()
    for i in range(2, n):
        if not Fwrite_displacement(file, superblock.s_inode_start + i * struct.calcsize(Inode().get_const()), inode):
            result += f'No se pudo escribir el Inodo {i} en el disco {path}\n'
            file.close()
            return False, result
    result += f'Se escribió la Tabla de Inodos en el disco {path}\n'

    # Write Block Table
    block = FileBlock()
    for i in range(2, 3 * n):
        if not Fwrite_displacement(file, superblock.s_block_start + i * struct.calcsize(FileBlock().get_const()), block):
            result += f'No se pudo escribir el Bloque {i} en el disco {path}\n'
            file.close()
            return False, result
    result += f'Se escribió la Tabla de Bloques en el disco {path}\n'
        
    # Write Root Folder
    if not Fwrite_displacement(file, superblock.s_inode_start, inode0):
        result += f'No se pudo escribir el Inodo 0 en el disco {path}\n'
        file.close()
        return False, result
    if not Fwrite_displacement(file, superblock.s_block_start, folderblock):
        result += f'No se pudo escribir el Bloque 0 en el disco {path}\n'
        file.close()
        return False, result
    result += f'Se escribió la Carpeta Raiz en el disco {path}\n'
    
    # Write User File
    if not Fwrite_displacement(file, superblock.s_inode_start + struct.calcsize(Inode().get_const()), inode1):
        result += f'No se pudo escribir el Inodo 1 en el disco {path}\n'
        file.close()
        return False, result
    if not Fwrite_displacement(file, superblock.s_block_start + struct.calcsize(FileBlock().get_const()), fileblock):
        result += f'No se pudo escribir el Bloque 1 en el disco {path}\n'
        file.close()
        return False, result
    result += f'Se escribió el Archivo de Usuarios en el disco {path}\n'
